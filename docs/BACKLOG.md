...
- [x] **1. (Wow moment) Live total entropy score on page load.** With zero clicks,
...
- [x] **2. Shannon entropy module with full test coverage.** Harden and extend
...
- [x] **3. Per-signal breakdown panel with plain-English explainers.** List every
...
- [x] **4. Design polish — the radar scope hero.** Implement `docs/DESIGN.md`'s radar
...
- [x] **5. Canvas and timezone/locale collectors.** `src/signals/canvas.js` renders a
...
- [x] **6. WebGL and audio-context collectors with safe fallbacks.** Read the unmasked
      ...
      - When `OfflineAudioContext` is undefined (mocked), the collector returns
        `"unavailable"` instead of throwing.
- [x] **7. Hardware/navigator misc collector.** Bundle languages, platform,
      ...
      - Missing properties (e.g. `deviceMemory` undefined on some browsers) default to
        ...
        path.
- [x] **8. Design polish — signal breakdown at every width.** Make the sidebar/list from
...
- [x] **9. Bundled population-frequency reference table.** Ship
...
- [x] **10. Combined uniqueness estimate.** Multiply independent per-signal
...
- [x] **11. Identifiability gauge.** Compare the user's total bits against the reference
      distribution and show a low/medium/high band.
      - A unit test maps bits values at each band boundary to the correct label
        ("Low"/"Medium"/"High").
- [x] **12. Design polish — gauge and caveat treatment.** Bring the gauge and combined
...
- [x] **13. "Nothing leaves your browser" privacy banner.** State and prove the
...
- [x] **14. Copy-to-clipboard summary export.** Let the user copy a plain-text summary of
